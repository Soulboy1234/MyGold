...
const STRATEGY_HISTORY = {
  ...
  versions: [
    ...
    },
    {
      ...
      updatedAt: "2026-03-10 16:35:00",
      title: "Long Horizon Fee-Aware Gold Allocator",
      titleZh: "长线低频黄金配置策略",
      changes: [
        "Clone Agent1's current portfolio state and historical records as the starting point.",
        "Raise score thresholds and lower target-position tiers to reduce trade frequency.",
        "Widen the rebalance buffer and minimum trade amount so only higher-conviction long-horizon adjustments are executed.",
      ],
      changesZh: [
        "以 Agent1 当前持仓状态和历史记录为模板，复制出独立分身。",
        "提高综合评分阈值并下调目标仓位档位，减少调仓频率。",
        "放宽再平衡缓冲区并提高最小交易金额，只执行更高确信度的长线调整。",
      ],
      reason: "Agent3 is designed to hold longer and trade less, so the same signal framework is kept but remapped into a slower, lower-turnover allocation profile.",
      reasonZh: "Agent3 目标是偏长线、少操作，因此保留综合信号框架，但把它映射成更慢、更低换手的仓位模型。",
    },
    {
      version: "v3.1.0",
      createdAt: "2026-03-11 07:20:00",
      updatedAt: "2026-03-11 07:20:00",
      title: "Long-Horizon Conviction Filter",
      titleZh: "长线高确信度过滤",
      changes: [
        "Raise entry thresholds again, lower target-position tiers and widen the rebalance buffer.",
        "Require larger net-profit cushions for ordinary trims while keeping protective reductions available.",
        "Add a much longer post-buy cooldown before normal long-horizon trims are allowed.",
      ],
      changesZh: [
        "进一步提高入场阈值、降低目标仓位档位，并放大再平衡缓冲。",
        "普通长线调仓卖出要求更厚的净利润缓冲，但继续保留保护性减仓。",
        "在长线策略里加入更长的买入后冷却，避免刚建仓不久就来回调仓。",
      ],
...
const STRATEGY = {
  id: "long-horizon-fee-aware-gold-allocator",
  version: STRATEGY_HISTORY.currentVersion,
  createdAt: "2026-03-10 16:35:00",
  name: "Long Horizon Fee-Aware Gold Allocator",
  nameZh: "多信号综合黄金配置策略",
  description: "Use Agent1's composite market signal model, but only react when the regime is strong enough to justify lower-turnover long-horizon reallocation.",
  descriptionZh: "把长期趋势、美元与 GLD 代理、真实利率和盘中偏离度合并成综合评分，再映射为目标仓位。",
  buyRule: [
    "daily composite score >= 58 for an initial probe, >= 68 for balanced size, >= 78 for strong size",
    "price stays above the long-term trend floor",
    "macro environment must remain broadly supportive",
    "small intraday swings are ignored unless the higher-level regime is already strong",
  ],
  sellRule: [
    "daily composite score < 20",
    "or price < SMA200 * 0.965",
    "or the long-term regime weakens enough to justify stepping back to lighter exposure",
  ],
  buyRuleZh: [
    "综合评分达到 48 分以上。",
    "价格维持在长期趋势底线之上。",
...
Object.assign(STRATEGY, {
  nameZh: "长线低频黄金配置策略",
  descriptionZh: "沿用 Agent1 的综合市场信号模型，但只有在更强、更明确的环境下才调仓，把策略改成偏长线、低换手的配置模型。",
  buyRuleZh: [
    "综合评分达到 58 分以上才建立试探仓，68 分以上进入平衡仓，78 分以上才进入强势仓。",
    "价格必须维持在长期趋势底线之上。",
    "宏观环境需要整体仍然支持黄金。",
    "普通盘中波动默认忽略，只有高层级环境已经足够强时才考虑顺势加仓。",
  ],
  sellRuleZh: [
    "综合评分跌破 20 分。",
    "或者价格跌破 SMA200 的 96.5%。",
    "或者长线环境明显转弱，此时才把仓位收缩回更保守的区间。",
  ],
  scoreMethodZh: [
    "综合评分 = 趋势分 + 宏观分 + 盘中位置分 + 追踪建议分，最终截断在 0 到 100 分。",
    "趋势分和宏观分仍是主导项，只有它们同时稳定偏强时，长线仓位才会逐步提升。",
    "盘中位置分仍然参与计算，但在本策略里只作为辅助，不会像短线策略那样频繁触发交易。",
    "仓位映射更保守：只有更高分数才允许逐级升仓，再平衡缓冲区也更大，因此整体换手率更低。",
  ],
});
...
const CONFIG = await loadStrategyConfig(FILES.strategyConfig, DEFAULT_CONFIG, {
  agentName: "agent3-长线选手",
  strategyVersion: STRATEGY_HISTORY.currentVersion,
  description: "长线选手自动策略参数。修改后会同时影响历史回测与实时决策。",
});
...
function applyChineseStrategyText() {
  const localizedVersions = new Map([
    ...
    ["v2.2.0", {
      ...
      changes: [
        "把综合评分阈值和目标仓位映射显式收敛到配置层。",
        "让实盘和回测共用同一套综合决策 helper，减少口径漂移。",
        "在考虑卖出手续费的历史验证后，收紧弱势区间退出并降低强势区间目标仓位。",
      ],
    ...
    ["v3.0.0", {
      ...
      reason: "Agent3 目标是偏长线、少操作，因此保留综合信号框架，但把它映射成更慢、更低换手的仓位模型。",
    }],
  ]);
  ...
  STRATEGY.name = "长线低频黄金配置策略";
  STRATEGY.nameZh = "长线低频黄金配置策略";
  STRATEGY.description = "沿用 Agent1 的综合市场信号模型，但只在更强、更明确的环境下才调仓，把策略改成长线、低换手的配置模型。";
  STRATEGY.descriptionZh = "沿用 Agent1 的综合市场信号模型，但只在更强、更明确的环境下才调仓，把策略改成长线、低换手的配置模型。";
  STRATEGY.buyRule = [
    "综合评分达到 58 分以上才建立试探仓，68 分以上进入平衡仓，78 分以上才进入强势仓。",
    "价格必须维持在长期趋势底线之上。",
    "宏观环境需要整体仍然支持黄金。",
  ...
  STRATEGY.scoreMethodTitle = "综合评分计算方式";
  STRATEGY.scoreMethodTitleZh = "综合评分计算方式";
  STRATEGY.scoreMethodZh = [
    "综合评分 = 趋势分 + 宏观分 + 盘中位置分 + 追踪建议分，最终截断在 0 到 100 分。",
    "趋势分 0 到 38 和宏观分 0 到 32 仍然是主导项，只有这两部分同时稳定偏强时，长线仓位才会逐步提升。",
    "盘中位置分 2 到 14 仍然参与计算，但在本策略里只作为辅助确认，不会像短线策略那样频繁触发交易。",
    "追踪建议分 0 到 14 用来微调方向判断：高频、日频和方向描述偏多时加分，观望或转弱时减分。",
    "仓位映射更保守：20 分以下明显防守，58 分以上才建立试探仓，68 分以上进入平衡仓，78 分以上才进入强势仓。",
    "本策略的再平衡缓冲更大、最小交易金额也更高，所以只有更高确定性的长线环境变化才会触发调仓。",
  ];
}
...
function buildRebalanceOrder({ latest, portfolio, currentPrice, target, recentTrade, config, diagnostics, backtest }) {
  ...
  if (shouldDelayNormalTrim(target.decisionKind, latest, recentTrade, realizedPnlCny, grams, config)) {
    return buildHoldOrder({
      latest,
      reason: `${target.reason} 但距离上一次买入还太近，当前更偏向长线持有，暂时不做普通调仓减仓。`,
  ...
  if (shouldSkipLowEdgeSell(target.decisionKind, realizedPnlCny, grams, config)) {
    return buildHoldOrder({
      latest,
      reason: `${target.reason} 但这笔长线调仓卖出在扣除手续费后净收益偏低，暂时不卖出。`,
      targetPositionRatio: currentRatio,
...
function runBacktest(rows, config) {
  ...
  return {
    ...
    notes: [
      ...
      "v3.0.0 验证了这套长线低换手阈值在卖出手续费约束下的可行性。",
    ],
