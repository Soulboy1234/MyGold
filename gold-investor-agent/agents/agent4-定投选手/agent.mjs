...
const CONFIG = await loadStrategyConfig(FILES.strategyConfig, DEFAULT_CONFIG, {
  agentName: "agent4-定投选手",
  strategyVersion: STRATEGY_HISTORY.currentVersion,
  description: "定投选手自动策略参数。修改后会同时影响历史回测与实时决策。",
});
...
function applyDcaStrategyTemplate() {
  ...
  STRATEGY.scoreMethodTitle = "定投与减仓规则说明";
  STRATEGY.scoreMethodTitleZh = "定投与减仓规则说明";
  STRATEGY.scoreMethodZh = [
    "本策略的主轴不是综合评分，而是固定节奏定投；综合评分只作为辅助观察，不直接驱动每日买入。",
    "每天本地时间 15:00 之后，如果当天还没有执行过定投且账户仍有现金，就固定买入 1000 元黄金。",
    "如果剩余现金不足 1000 元，则最后一次投入全部剩余现金，之后停止继续定投。",
    "卖出只有两类场景：一类是扣掉手续费后的净盈利大约达到 8%，且价格明显高于盘中均线时做低频止盈；另一类是长期趋势明显转弱时做防守减仓。",
    "卖出至少间隔 7 天，避免为了短线波动频繁交易，让定投策略仍然保持低换手和纪律性。",
  ];
}
...
function decideAndApply(context) {
  ...
  const nextState = {
    ...
    lastDcaTradeDate: isBuyAction(order.action)
      ? today
      : context.persisted.agentState.lastDcaTradeDate || null,
    lastDcaSellDate: isSellAction(order.action)
      ? today
      : context.persisted.agentState.lastDcaSellDate || null,
  };
